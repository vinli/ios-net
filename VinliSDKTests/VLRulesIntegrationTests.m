...
- (void)setUp {
    ...
    [[VLSessionManager sharedManager].service startWithHost:self.accessToken requestUri:[NSString stringWithFormat:@"https://rules.vin.li/api/v1/rules/%@", self.ruleId] onSuccess:^(NSDictionary *result, NSHTTPURLResponse *response) {
    ...
    [[VLSessionManager sharedManager].service startWithHost:self.accessToken requestUri:[NSString stringWithFormat:@"https://rules.vin.li/api/v1/devices/%@/rules", self.deviceId] onSuccess:^(NSDictionary *result, NSHTTPURLResponse *response) {
