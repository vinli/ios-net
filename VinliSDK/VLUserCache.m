...
- (void)encodeWithCoder:(NSCoder *)encoder {
	...
	NSError *error;
	[encoder encodeObject:[NSKeyedArchiver archivedDataWithRootObject:_user requiringSecureCoding:FALSE error:&error] forKey:@"user"];
}
...
- (id)initWithCoder:(NSCoder *)decoder {
	if((self = [super init])) {
		...
		NSError *error;
		_user = [NSKeyedUnarchiver unarchivedObjectOfClass:[VLUser class] fromData:[decoder decodeObjectForKey:@"user"] error:&error];
	}
	return self;
}
...
{
	NSError *error;
	NSData *encodedObject = [NSKeyedArchiver archivedDataWithRootObject:self requiringSecureCoding:FALSE error:&error];
